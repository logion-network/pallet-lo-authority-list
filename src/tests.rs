...
fn it_adds_lo() {
	new_test_ext().execute_with(|| {
		assert_ok!(LoAuthorityList::add_legal_officer(Origin::root(), LEGAL_OFFICER_ID, Default::default()));
...
fn it_removes_lo() {
	new_test_ext().execute_with(|| {
		assert_ok!(LoAuthorityList::add_legal_officer(Origin::root(), LEGAL_OFFICER_ID, Default::default()));
		assert_ok!(LoAuthorityList::remove_legal_officer(Origin::root(), LEGAL_OFFICER_ID));
...
fn it_lets_manager_update() {
	new_test_ext().execute_with(|| {
		...
		let base_url = "https://node.logion.network".as_bytes().to_vec();
		let node_id = OpaquePeerId(bs58::decode("12D3KooWBmAwcd4PJNJvfV89HwE48nwkRmAgo8Vy3uQEyNNHBox2").into_vec().unwrap());
		assert_ok!(LoAuthorityList::update_legal_officer(Origin::root(), LEGAL_OFFICER_ID, LegalOfficerData {
			node_id: Option::Some(node_id.clone()),
			base_url: Option::Some(base_url.clone()),
		}));
		assert_eq!(LoAuthorityList::legal_officer_set(LEGAL_OFFICER_ID).unwrap().base_url.unwrap(), base_url);
...
fn it_fails_add_if_peer_id_already_in_use() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(LoAuthorityList::add_legal_officer(Origin::root(), LEGAL_OFFICER_ID, LegalOfficerData {
		...
		assert_err!(LoAuthorityList::add_legal_officer(Origin::root(), LEGAL_OFFICER_ID2, LegalOfficerData {
...
fn it_fails_update_if_peer_id_already_in_use() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(LoAuthorityList::add_legal_officer(Origin::root(), LEGAL_OFFICER_ID2, LegalOfficerData {
			base_url: Option::Some(base_url2.clone()),
		...
		assert_err!(LoAuthorityList::update_legal_officer(Origin::root(), LEGAL_OFFICER_ID2, LegalOfficerData {
			base_url: Option::Some(base_url2.clone()),
			node_id: Option::Some(node_id1.clone()),
		}), Error::<Test>::PeerIdAlreadyInUse);
	});
